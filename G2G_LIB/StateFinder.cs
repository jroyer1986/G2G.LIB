...
namespace G2G_LIB
{
    public class StateFinder
    {
        public static string GetState(string zipCode)
        {
            ...
            try
            {
                if (zipCode.GetType() != typeof(string))
                {
                    Console.WriteLine("Must pass the zipcode as a string");
                }
                if (zipCode.Length != 5)
                {
                    Console.WriteLine("Must pass 5-digit zipcode");
                }

                int thiszip = int.Parse(zipCode, System.Globalization.NumberStyles.Any);
            ...
            {
                throw new Exception("Could not find a State Code.");
            }
        }
    }
}
